...
function b(){
	...
	for(var i=0;i<links.length;i++){
		links[i].onmouseover=function(){
			...
			move("img_con",0,0,5);
			if(this.getAttribute("href").indexOf("beauty.html")!=-1)
			move("img_con",-150,0,5);
			if(this.getAttribute("href").indexOf("sweet.html")!=-1)
			move("img_con",-300,0,5);
			if(this.getAttribute("href").indexOf("love.html")!=-1)
...
function c(){ //还有一种解决方案
	if(!document.getElementById("beauty")) return false;
	var jq=document.getElementById("beauty");
...
function d(){
	...
	if(!document.getElementById("sweet")) return false;
	...
	var jn=document.getElementById("sweet");
	insertAfter(p,jn);
	insertAfter(img,p);
...
function f(){
	...
	h.appendChild(h_txt);
	var ph=document.getElementById("love");
	ph.appendChild(h);
